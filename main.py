...
def get_args_parser():
    ...
    parser.add_argument("--loss_weights", type=float, nargs="+", default=[1.0, 1.0, 1.0], help="""three loss weights for instance, local-group, group supervision losses in turn""")
    parser.add_argument("--use_bn_in_head", type=utils.bool_flag,default=False, help="Whether to use batch normalizations in the three projection heads (Default: False)")
    parser.add_argument("--norm_before_pred", type=utils.bool_flag, default=True,
        help="""Whether to use batch normalizations after projection heads (namely before prediction heads) in instance and local-group supervisions. (Default: False)""")

    # parameters for instance discrimination supervision
    parser.add_argument("--instance_out_dim", type=int, default=256, help="""output dimention in the projection and prediction heads.""")
    parser.add_argument("--instance_queue_size", type=int, default=65536, help="""the queue size of the memory to store the negative keys.""")
    parser.add_argument("--instance_temp", type=float, default=0.2, help="""the temperature parameters for the infoNCE loss in instance supervision.""")

    # parameters for local-group discrimination supervision
    parser.add_argument("--local_group_out_dim", type=int, default=256, help="""output dimention in the projection and prediction heads.""")
    parser.add_argument("--local_group_knn_top_n", type=int, default=8, help="how many neighbors we use to aggregate for a local-group")
    parser.add_argument(
